...
class EcwidAPI():
	...
	def GetStoreProducts(self, store_id, token, **kwargs):
		...
		response = get(self._REST_API_URL.format(store_id = store_id,endpoint = 'products'), params = params)
		if response.status_code != 200:
			raise InvalidAPIKeys
	...
	def GetStoreOrders(self, store_id, token, **kwargs):
		...
		params = {'token':token, **kwargs}
		response = get(self._REST_API_URL.format(store_id = store_id,endpoint = 'orders'), params = params)
		if response.status_code != 200:
			raise InvalidAPIKeys
		return response.json()
